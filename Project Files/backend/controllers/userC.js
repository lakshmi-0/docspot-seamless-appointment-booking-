...
// ----------- NEW: Book Appointment Controller -----------
const bookAppointmentController = async (req, res) => {
  try {
    const { fullName, phone, date, userId } = req.body;
    let documentData = null;
    if (req.file) {
      documentData = {
        filename: req.file.filename,
    ...
    await newAppointment.save();
    res.json({ success: true, message: "Successfully booked appointment" });
  } catch (error) {
    console.log(error);
...
// ----------- NEW: Get Appointments For User Controller -----------
const getAppointmentsForUserController = async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({ success: false, data: [], message: "Failed to fetch appointments" });
  }
};
...
module.exports = {
  ...
  getDocsController,
  bookAppointmentController, // <-- export new controller
  getAppointmentsForUserController // <-- export new controller
};